...
//    NSDictionary *userDic = (NSDictionary *)[cache objectForKey:KUserModelCache];
    ...
    return userDic[@"third_return_params"];
}
        ...
        [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
            switch (status) {
                ...
                case AFNetworkReachabilityStatusNotReachable:
                    ...
                    [MBProgressHUD hidden:GetCurrentValidViewCon().view];
                    [MBProgressHUD showNetworkDisconnectMessage:@"网络断开连接,请检查网络设置" View:GetCurrentValidViewCon().view];
                    break;
...
+ (NSURLSessionTask *)POST:(NSString *)URL
    ...
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:parameters];
    [parameter setValue:@"mobile" forKey:@"platform"];
    return [self POST:URL serviceName:service parameters:parameter responseCache:nil success:success failure:failure];
}
...
+ (NSURLSessionTask *)POSTWithJSON:(NSString *)URL
    ...
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:parameters];
    [parameter setValue:@"mobile" forKey:@"platform"];
    return [self POSTWithJSON:URL serviceName:service parameters:parameter responseCache:nil success:success failure:failure];
}
...
+ (NSURLSessionTask *)PUT:(NSString *)URL
    ...
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:parameters];
    [parameter setValue:@"mobile" forKey:@"platform"];
    return [self PUT:URL serviceName:service parameters:parameter responseCache:nil success:success failure:failure];
}
...
+ (NSURLSessionTask *)PATCH:(NSString *)URL
    ...
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:parameters];
    [parameter setValue:@"mobile" forKey:@"platform"];
    return [self PATCH:URL serviceName:service parameters:parameter responseCache:nil success:success failure:failure];
}
...
+ (NSURLSessionTask *)DELETE:(NSString *)URL
    ...
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:parameters];
    [parameter setValue:@"mobile" forKey:@"platform"];
    return [self DELETE:URL serviceName:service parameters:parameter responseCache:nil success:success failure:failure];
}
...
+ (NSURLSessionTask *)GET:(NSString *)URL
    ...
    //加密 header body
    [self encodeParameters:parameter];
    // 设置 header
    [self setHeaderWithServiceName:service];
    NSURLSessionTask *sessionTask = [_sessionManager GET:URL parameters:parameter progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (![self tokenInvalidWithCode:responseObject[@"respCode"]]) {
            return;
        }
...
+ (NSURLSessionTask *)POST:(NSString *)URL
    ...
    // 设置header
    [self setHeaderWithServiceName:service];
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:parameter progress:^(NSProgress * _Nonnull uploadProgress) {} success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (_isOpenAES) {
            //解密
            responseObject = aesDecryptWithData(responseObject);
        }
...
+ (NSURLSessionTask *)PUT:(NSString *)URL
    ...
    // 设置header
    [self setHeaderWithServiceName:service];
    NSURLSessionTask *sessionTask = [_sessionManager PUT:URL parameters:parameter success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (_isOpenAES) {
            //解密
            responseObject = aesDecryptWithData(responseObject);
        }
...
+ (NSURLSessionTask *)PATCH:(NSString *)URL
    ...
    // 设置header
    [self setHeaderWithServiceName:service];
    NSURLSessionTask *sessionTask = [_sessionManager PATCH:URL parameters:parameter success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (_isOpenAES) {
            //解密
            responseObject = aesDecryptWithData(responseObject);
        }
...
+ (NSURLSessionTask *)DELETE:(NSString *)URL
    ...
    // 设置header
    [self setHeaderWithServiceName:service];
    NSURLSessionTask *sessionTask =  [_sessionManager DELETE:URL parameters:parameter success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (_isOpenAES) {
            //解密
            responseObject = aesDecryptWithData(responseObject);
        }
...
#pragma mark - **************** Token失效跳转回登录页面

+ (BOOL)tokenInvalidWithCode:(NSString *)code{
    
    if ([code isEqualToString:@"S90004"]||[code isEqualToString:@"S20308"]||[code isEqualToString:@"S20307"] ) {
            //清除登录数据 - 退出登录
        KPostNotification(KNotificationLoginStateChange, @NO);
        [MBProgressHUD hidden:GetCurrentValidViewCon().view];
        [MBProgressHUD showWarningMessage:@"长时间未登录,考虑您的账号安全,请重新登录" View:GetCurrentValidViewCon().view];
        return NO;
    }
    return YES;
}
...
#pragma mark - - KPostNotification(KNotificationLoginStateChange, @NO) - - 弃用
...
+ (void)setHeaderWithServiceName:(NSString *)service {
    if([PPNetworkHelper getRequestToken] && [PPNetworkHelper getRequestToken].length > 0){
        // json形式提交数据
        //配置token
        NSString *token = [PPNetworkHelper getRequestToken];
        ...
        [_sessionManager.requestSerializer setValue:token forHTTPHeaderField:@"_idp_session"];
    }else{
        // 未登录状态下清除token
        [_sessionManager.requestSerializer setValue:nil forHTTPHeaderField:@"_idp_session"];
    }
    // 登录接口使用form 形式提交数据
    //    [_sessionManager.requestSerializer setValue:service forHTTPHeaderField:@"service_name"];
    [_sessionManager.requestSerializer setValue:[FCUUID uuidForDevice] forHTTPHeaderField:@"X-ATT-DeviceId"];
    [_sessionManager.requestSerializer setValue:[FCUUID uuidForDevice] forHTTPHeaderField:@"X-Request-ID"];
...
+ (NSURLSessionTask *)uploadFileWithURL:(NSString *)URL
    ...
    if ([PPNetworkHelper getRequestToken] && [PPNetworkHelper getRequestToken].length > 0){
        // json形式提交数据
        [_sessionManager.requestSerializer setValue:[PPNetworkHelper getRequestToken] forHTTPHeaderField:@"_idp_session"];
    }
    ...
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:parameter constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //获取文件的后缀名
...
+ (NSURLSessionTask *)uploadImagesWithURL:(NSString *)URL
    ...
    [_sessionManager.requestSerializer setValue:@"fileupload" forHTTPHeaderField:@"format"];
